...
if [ "$PANEL_CHOICE" = "waybar" ]; then
    systemctl --user restart waybar-idle-monitor.service waypaper-watcher.service background-watcher.service hyprlock-watcher.service rofi-font-watcher.service cursor-theme-watcher.service &>/dev/null # waybar.service
else
    ...
    case "$reboot_choice" in
        [nN][oO]|[nN])
            ...
            if [ "$PANEL_CHOICE" = "waybar" ]; then
                ...
                systemctl --user restart waybar.service &>/dev/null && rm -rf "$HOME/hyprcandyinstall"
            else
                systemctl --user stop waybar.service &>/dev/null
                systemctl --user restart hyprpanel.service &>/dev/null && rm -rf "$HOME/hyprcandyinstall"
            fi
            ;;
        *)
            if [ "$PANEL_CHOICE" = "waybar" ]; then
            ...
            sudo reboot && rm -rf "$HOME/hyprcandyintall"
            ;;
    esac
}
